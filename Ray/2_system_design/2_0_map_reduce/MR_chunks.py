...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=positive_int,
        default=20,
        help="How many times to repeat the mini-corpus."
    )
    parser.add_argument(
        "--docs-file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=positive_int,
        default=12,
        help="How many global words to print."
    )
    parser.add_argument(
        "--address",
        type=str,
...
def count_words_with_delay(chunk_id: int, docs: list[list[str]], slow_id: int, straggler_delay_s: float) -> tuple[int, *Counter[str]]:
...
def main() -> None:
    ...
    print(f"corpus_size       = {len(corpus)} docs | chunks={len(chunks)} | reducers={NUM_REDUCERS} | reduce_batch_size={args.reduce_batch_size}")
    print(f"slow_chunk_id     = {slow_id} | straggler_delay={args.straggler_delay_s:0.2f}s | reduce_delay_per_bucket={args.reduce_delay_per_bucket_s:0.2f}s")
    print(f"{banner}\n")
    ...
    while pending_chunks:
        ...
        for reducer_id in range(NUM_REDUCERS):
            if len(ready_bucket_refs[reducer_id]) >= args.reduce_batch_size:
                ...
                partial_reduce_refs[reducer_id].append(partial_reduce.remote(args.reduce_delay_per_bucket_s, *batch))
                print(f"launched partial reduce for reducer {reducer_id} with {len(batch)} ready buckets")
    ...
    for reducer_id in range(NUM_REDUCERS):
        if ready_bucket_refs[reducer_id]:
            batch = ready_bucket_refs[reducer_id]
            partial_reduce_refs[reducer_id].append(partial_reduce.remote(args.reduce_delay_per_bucket_s, *batch))
            print(f"final partial reduce for reducer {reducer_id} with {len(batch)} ready buckets")
            ready_bucket_refs[reducer_id] = []
