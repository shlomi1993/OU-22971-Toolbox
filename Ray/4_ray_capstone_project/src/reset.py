...
def reset_ray(prepared_dir: Path, output_dir: Path) -> None:
    ...
    """
    # Stop Ray
    g_logger.info("Stopping Ray...")
    try:
        subprocess.run(["ray", "stop", "--force"], capture_output=True, timeout=30)
        g_logger.info("Ray stopped successfully")
    except subprocess.TimeoutExpired:
        g_logger.warning("Ray stop command timed out, continuing with cleanup...")
    except FileNotFoundError:
        g_logger.warning("Ray command not found, skipping Ray shutdown")
    except Exception as e:
        g_logger.warning(f"Failed to stop Ray: {e}, continuing with cleanup...")

    # Remove prepared assets
    if prepared_dir.exists():
        shutil.rmtree(prepared_dir)
        g_logger.info(f"Prepared assets removed: {prepared_dir}")
    else:
        g_logger.info("No prepared assets to remove")

    # Remove output artifacts
    if output_dir.exists():
        shutil.rmtree(output_dir)
        g_logger.info(f"Output artifacts removed: {output_dir}")
    else:
        g_logger.info("No output artifacts to remove")

    g_logger.info("Reset complete")
