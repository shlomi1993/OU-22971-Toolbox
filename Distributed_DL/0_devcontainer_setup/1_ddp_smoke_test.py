...
This script is the quick environment check for the rest of the part. Each rank builds the same tiny MLP, runs one
forward pass, one backward pass, and one optimizer step under ``DistributedDataParallel``, then rank 0 gathers a short
status summary from every process.

On a GPU machine it prefers CUDA with NCCL. Inside the local CPU-only dev container it falls back to CPU with Gloo, so
the same command is still useful before moving on to the later lessons.
...
def setup() -> tuple[int, int, int, torch.device, str]:
...
def format_status_report(statuses: list[dict]) -> str:
