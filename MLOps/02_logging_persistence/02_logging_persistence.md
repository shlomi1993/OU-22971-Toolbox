...
# 02 Experiment Logging and Artifact Persistence

## Setup

Activate the conda environment created in **01**:
...
## Logging
As is, `ml_pipeline.py` writes its outputs to fixed filenames, which means that the only artifacts we keep are the last run's outputs.
...
3. **Execute `ml_pipeline.py` from its original location**
   ...
   Conceptually:
   ```bash
   python ../01_conda_environments/ml_pipeline.py \
     --data ../01_conda_environments/data/clean.csv
   ```
...
**Cons**
- Fragile by construction: breaks if folder layout and script names changes.
- Debugging is brittle - good luck reproducing a failure without the exact same filesystem state.
...
As is, our scripts output metrics and plots. What if we want to use the fitted model on new data?
...
Experiment tracking systems solve these problems in a more disciplined, thought-out way.
