...
Activate the environment from **01**:
...
- a backend that isn't "a folder naming convention"

Our `logging_wrapper.py` from **02** was a toy version of this idea.
...
MLflow is basically a **runs database with a UI**.

A few core cogs:

- **Tracking server**: a small HTTP service that serves the UI and accepts logs
  - think: a queryable version of the `runs/` folder from `02_conda_logging_persistence`.

- **Experiment**: a named bucket of runs (project / task)
  - example: all runs produced while working on `02_conda_logging_persistence`
...
In **04**, we'll modify `ml_pipeline.py` so it logs:
