...
def save_scatter(X: np.ndarray, y: np.ndarray, title: str, path: str) -> None:
    plt.figure()
...
def main() -> None:
    ...
    # Save plots
    save_scatter(X, y, title="Original data", path=os.path.join(args.outdir, "original.png"))
    save_scatter(X_clean, y_clean, title=f"Cleaned data (cutoff = ±{args.cutoff})", path=os.path.join(args.outdir, "clean.png"))
