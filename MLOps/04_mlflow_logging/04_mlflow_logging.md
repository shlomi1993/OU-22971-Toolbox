...
2. generate **two distinct data sets**. Run:
   
   ```bash
   python generate_data.py --seed 0 --cutoff 10 --outdir data1
   ```
    and then, with a **different RNG seed**:
...
1. Right click `01_conda_environments\ml_pipeline.py`
2. click **"Select for Compare"**.
3. Right click `04_mlflow_logging\ml_pipeline_logging.py`
...
```
**Big takeaway:** ML code stays the same.
