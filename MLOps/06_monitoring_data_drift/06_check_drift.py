...
Unit 06: monitoring pass (integrity + drift + degradation).
...
Example:
  python 06_check_drift.py --ref-parquet TLC_data/green_tripdata_2020-01.parquet --cur-parquet TLC_data/green_tripdata_2020-04.parquet --simulate-issues
"""
...
def main() -> None:
    ...
    df_ref_raw = load_taxi_table(args.ref_parquet)
    df_cur_raw = load_taxi_table(args.cur_parquet)
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        ds_ref = mlflow.data.from_pandas(
            df_ref_raw,
            source=LocalArtifactDatasetSource(str(args.ref_parquet)),
            name="raw_reference",
        )
        ds_cur = mlflow.data.from_pandas(
            df_cur_raw,
            source=LocalArtifactDatasetSource(str(args.cur_parquet)),
            name="raw_current",
        )
