# MLOps Capstone Project Design Doc
## Manual monitoring → optional retraining → champion promotion (Metaflow + NannyML + MLflow)

This document specifies the Unit 06 MLOps capstone project.
...
- transforms raw columns into model-ready features:
  ...
  - location features (zone IDs, one-hot/target encoding, Latitude \ Longitude or embeddings if you go fancy)
...
Decide retrain (Explore NannyML docs for decision rules):
...
- it doesn’t win via obvious overfit or leakage
...
- OR require “doesn’t regress reference by > Y%”
...
Goal: Trigger the flow automatically upon ingestion of a new data slice.
...
2. **Short video (5–10 min)** showing:
   ...
   - **Inference demo (offline, Unit 06):**
...
**Metaflow** is an accesible solution.
